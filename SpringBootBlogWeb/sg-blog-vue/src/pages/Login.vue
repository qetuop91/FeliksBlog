    ...
    export default {
        ...
        data() { //选项 / 数据
            return {
                ...
                loginErr: false,//登录错误
                loginTitle:'用户名或密码错误',
                nphonenumberErr:false,//新用户注册手机号格式错误
        ...
        methods: { //事件处理器
            ...
            newRegister:function(){//注册提交
                ...
                if(phreg.test(that.nphonenumber)){
                ...
                if(!that.nusernameErr&&!that.nemailErr&&!that.npasswordErr&&!that.nphonenumberErr){
                    that.fullscreenLoading = true;
                    userRegister(that.nusername,that.nnickName,that.nphonenumber,that.nemail,that.npassword).then((response)=>{
                         //注册成功后调整到登录
                         that.goLogin()
                    }).catch((error)=>{
